...
def barangay_clearance(request):
    if request.method == "POST":
        # Extract form data
        data = {
            'full_name': request.POST.get('firstname') + ' ' + request.POST.get('middlename') + ' ' + request.POST.get('lastname'),
        ...
        for proof_file in proof_files:
            file_name = default_storage.save(f'proof_of_residency/{proof_file.name}', proof_file)
            clearance_request.proof_of_residency = file_name
            clearance_request.save()
...
def update_request_status(request, request_id):
    ...
    if new_status in ['Approved', 'Declined']:
        ...
        # Create notification
        notification_message = f"Your {clearance_request.document_type} request has been {new_status.lower()}."
        if new_status == 'Approved':
            notification_message += " You can claim it at the barangay office."
        elif new_status == 'Declined':
            notification_message += " Reason: You are not a resident of this barangay."
